...
**2、添加现有算法不同版本:**
	当需要对项目中**已有的算法文件**，添加自己的算法版本时，请在自己的名字空间中，以大写字母'C'+'文件名'声明类，并在类中实现自己的算法，另外需要实现一个**静态的测试接口 test()**；
...
**3、添加新算法:**
	当需要添加新算法时，请创建对应算法名的头文件，并以2中的规则添加名字空间以及类；同时，请在[Args.h](./code/Algs.h)中引用该头文件，并加以简单的注释说明;

**4、main中调用对应的算法：**
	直接调用对应名字空间中需要的算法test函数;
